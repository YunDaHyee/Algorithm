 ...
 * 
	...
	@History
		...
		다했는데 런타임에러가 난다.. 왤까? 하루종일 이것만 했는데...하
		-> 틀린이유 : 1로 나오고 시스템 종료 되기 전에 bw.flush() 안해줘서..;;; 출력이 안되어서 틀렸다고 뜬 듯..
...
public class _13023 {
	public static void main(String args[]) throws IOException{
		...
		List<Integer>[]			adjacencyList	= new ArrayList[N];			// 인접리스트.
																			/*기록3 - 배열의 초기화를 첨 깨닫다; 근데 해줘도 인덱스 접근이 안됨..ㅠ
																			 * 배열로 이요하고자 할 떄는 리스트 함수를 쓰더라도 배열을 쓰자. List<Integer>[] 일케.
																			 * 그러면 사이즈를 지정함과 동시에 인덱스로의 접근이 가능해진다!!
																			 * 나는 int/string 이런것만 해야할줄알았는데 미처 생각을 그까지 못했다!꿀팁이다!!
																			 * */
		...
		for( int i=0;i<M;i++ ){
			...
			adjacencyList[from].add(to); /* 기록2. 초기화가 안됐는데 어떻게 인덱스로 접근해야할까..
											정점들이 많을지 모르니까 초기화를 내 맘대로 할 수도 없다
											<-이게 고민이었는데 기록3을 통해 깨닳음. 그러면 [ [1],[],[],[3,1],[] ] 이런형태가 돼!
										 */  
