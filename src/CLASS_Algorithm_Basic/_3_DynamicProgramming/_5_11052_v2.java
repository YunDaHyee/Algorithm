...
/**
	...
	@history
		...
		for( int i=1;i<cardPackPrice.length;i++ ){
			memo[i] = TopDown(n-i);
			memo[n] = Math.max( TopDown(n), memo[i]+cardPackPrice[i] );
		}
		...
		=> 대안 : 
				for( int i=1;i<cardPackPrice.length;i++ ){
					...
					memo[i] = TopDown(n-i); 
					memo[n] = Math.max( memo[n], memo[i]+cardPackPrice[i] );
				}
		...
		=> 대안 :
			...
			for( int i=1;i<cardPackPrice.length;i++ ){
				...
				max = Math.max( max, TopDown(n-i)+cardPackPrice[i] );
			}
...
public class _5_11052_v2 {
	private static int[] memo = new int[1001], cardPackPrice=new int[1001];
	...
	public static void main(String[] args) throws IOException {
		...
		for( int i=1;i<=buyCardCount;i++ ){
			cardPackPrice[i] = Integer.parseInt(raws[i-1]);
		}
	...
	private static int BottomUp(int n) {
		for( int i=1;i<=n;i++ ){
			for( int j=1;j<=i;j++ ){
                ...
                memo[i] = Math.max( memo[i], memo[i-j] + cardPackPrice[j] );
			}
		}
		return memo[n];
	}
